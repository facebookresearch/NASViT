...
model_urls = {
    ...
    'resnext101_32x4d': 'https://download.pytorch.org/models/resnext50_32x4d-7cdf4587.pth',
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer=norm_layer, is_last=False))
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # local_x = self.fc( F.adaptive_avg_pool2d(x, (2, 2)).permute(0, 2, 3, 1).reshape(-1, C) )
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc(x)

        return x, outputs # , local_x
